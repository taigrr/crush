...
func runNonInteractive(
	...
	// Mint a per-call RunID so we can correlate the terminal RunComplete
	// with *this* SendMessage even if the session was busy and another
	// turn finished first. Send in background so the event loop can
	// process streaming updates as they arrive.
	runID := uuid.New().String()
	sendErr := make(chan error, 1)
	go func() {
		sendErr <- c.SendMessage(ctx, ws.ID, sess.ID, runID, prompt)
	}()
