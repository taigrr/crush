...
func configureStdioProcess(cmd *exec.Cmd) {
	...
	cmd.Cancel = func() error {
		...
		// ESRCH means the group is already gone (the ordinary case on a clean
		// teardown), so treat it as success rather than surfacing a spurious
		// "no such process" warning out of cmd.Wait.
