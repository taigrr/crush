...
func TestToolErrorContract(t *testing.T) {
	...
	cases := []toolCase{
		...
		{
			name:  "replace_symbol missing file_path",
			tool:  NewReplaceSymbolTool(nil, perms, hist, ft, wd),
			ctx:   sessionCtx,
			call:  fantasy.ToolCall{ID: "c8a", Name: ReplaceSymbolToolName, Input: mustJSON(ReplaceSymbolParams{Symbol: "Foo"})},
			match: "file_path is required",
		},
		{
			name:  "replace_symbol missing symbol",
			tool:  NewReplaceSymbolTool(nil, perms, hist, ft, wd),
			ctx:   sessionCtx,
			call:  fantasy.ToolCall{ID: "c8b", Name: ReplaceSymbolToolName, Input: mustJSON(ReplaceSymbolParams{FilePath: "a.go"})},
			match: "symbol is required",
		},
		{
			name:  "replace_symbol invalid operation",
			tool:  NewReplaceSymbolTool(nil, perms, hist, ft, wd),
			ctx:   sessionCtx,
			call:  fantasy.ToolCall{ID: "c8c", Name: ReplaceSymbolToolName, Input: mustJSON(ReplaceSymbolParams{FilePath: "a.go", Symbol: "Foo", Operation: "bogus", NewContent: "x"})},
			match: "invalid operation",
		},
		{
			name:  "replace_symbol missing new_content",
			tool:  NewReplaceSymbolTool(nil, perms, hist, ft, wd),
			ctx:   sessionCtx,
			call:  fantasy.ToolCall{ID: "c8d", Name: ReplaceSymbolToolName, Input: mustJSON(ReplaceSymbolParams{FilePath: "a.go", Symbol: "Foo", Operation: "replace"})},
			match: "new_content is required",
		},
		{
...
func (stubSwarmBackend) CreateSessionInWorkspace(ctx context.Context, workspaceID string, _ SwarmNewOptions) (session.Session, error) {
...
func (stubSwarmBackend) CreateSessionInWorkspaceAtPath(ctx context.Context, path string, _ SwarmNewOptions) (string, session.Session, error) {
	return "", session.Session{}, nil
}
