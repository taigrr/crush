...
func (a *sessionAgent) Run(ctx context.Context, call SessionAgentCall) (result *fantasy.AgentResult, retErr error) {
	...
	// Queue the message if busy. Use Update so the read-modify-write of
	// the queue is atomic; otherwise concurrent enqueues (or an enqueue
	// racing with the drain in PrepareStep) can silently drop messages.
	// Strip OnComplete: the caller that supplied the hook (typically
	// coordinator.Run) has its own retry/coalesce scope that ends when
	// it returns, so by the time the queue drains nobody is left to
	// consume the buffered terminal event. The recursive Run will fall
	// back to the default broker publish, which is what existing
	// subscribers expect for queued turns.
