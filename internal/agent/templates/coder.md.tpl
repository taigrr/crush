...
2. Assemble the diff to review. The reviewers see ONLY what you pass, so it must contain the whole change — including work you already committed as you went. Plain `git diff` is almost never right (it shows only unstaged edits and is empty after a commit). Inspect the repo state and pick the command that captures the full cumulative change; do not assume a layout:
   ...
   - **No git at all** (not a repo — see the `<env>` "Is directory a git repo" flag — or a repo with zero commits where `git diff HEAD` fails): there is no diff to compute. Instead send the actual code you wrote/changed as the payload — the `review` tool's `diff` field also accepts raw code. Concatenate the relevant new/modified files with clear `// file: path` headers (or paste the functions you added) so the reviewers know what to inspect; they can read the rest of the tree with their own tools. Note in the `goal` that this is a new/un-versioned project so they don't expect diff syntax.
   - Sanity-check the result: if the diff comes back empty or clearly smaller than the work you did, you chose the wrong base (or there's no git) — widen it, switch to sending file contents, and retry before calling `review`.
   - If the diff is very large, still send it whole; only split by directory/subsystem across multiple `review` calls if it truly exceeds one call.
3. Call `review` once with that full `diff`, the original `goal`, and any `focus` areas. This fires both reviewers in parallel and returns both reports.
...
5. If the fixes were substantial, run `review` again on the updated diff. Repeat until the reviewers surface no real defects, then finish (create the PR, etc.).
