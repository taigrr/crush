...
func TestConfig_setupAgentsWithNoDisabledTools(t *testing.T) {
	...
	require.True(t, ok)
	assert.Equal(t, []string{"agentic_fetch", "glob", "grep", "ls", "multi_view", "sourcegraph", "context7", "search_history", "list_sessions", "view", "editor_context", "show_locations"}, taskAgent.AllowedTools)
}
...
func TestConfig_setupAgentsWithDisabledTools(t *testing.T) {
	...
	assert.Equal(t, []string{"agent", "bash", "crush_info", "crush_logs", "reload_config", "job_output", "job_kill", "multiedit", "lsp_diagnostics", "lsp_references", "lsp_definition", "lsp_document_symbols", "lsp_rename", "lsp_restart", "fetch", "agentic_fetch", "glob", "ls", "multi_view", "sourcegraph", "context7", "search_history", "list_sessions", "todos", "view", "write", "list_mcp_resources", "read_mcp_resource", "editor_context", "show_locations", "review", "swarm", "workspace_lookup", "rename_session", "question"}, coderAgent.AllowedTools)

	taskAgent, ok := cfg.Agents[AgentTask]
	require.True(t, ok)
	assert.Equal(t, []string{"agentic_fetch", "glob", "ls", "multi_view", "sourcegraph", "context7", "search_history", "list_sessions", "view", "editor_context", "show_locations"}, taskAgent.AllowedTools)
}
...
func TestConfig_setupAgentsWithEveryReadOnlyToolDisabled(t *testing.T) {
	...
	coderAgent, ok := cfg.Agents[AgentCoder]
	require.True(t, ok)
	assert.Equal(t, []string{"agent", "bash", "crush_info", "crush_logs", "reload_config", "job_output", "job_kill", "download", "edit", "multiedit", "lsp_diagnostics", "lsp_references", "lsp_definition", "lsp_document_symbols", "lsp_rename", "lsp_restart", "fetch", "agentic_fetch", "list_sessions", "todos", "write", "list_mcp_resources", "read_mcp_resource", "review", "swarm", "workspace_lookup", "rename_session", "question"}, coderAgent.AllowedTools)

	taskAgent, ok := cfg.Agents[AgentTask]
	require.True(t, ok)
	assert.Equal(t, []string{"agentic_fetch", "list_sessions"}, taskAgent.AllowedTools)
}
