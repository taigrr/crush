...
func modelRefConfig() *Config {
	...
	return &Config{
		...
		Models: map[SelectedModelType]SelectedModel{
			...
			SelectedModelTypeOrchestrator: {Provider: "dp-claude", Model: "claude-fable-5-1", ReasoningEffort: "low"},
...
func TestResolveModelRef_Roles(t *testing.T) {
	...
	got, err := c.ResolveModelRef("orchestrator")
...
func TestOrchestratorModelAndRoles(t *testing.T) {
	t.Parallel()
	c := modelRefConfig()
	sel, ok := c.OrchestratorModel()
	require.True(t, ok)
	require.Equal(t, "claude-fable-5-1", sel.Model)
	require.Equal(t, []SelectedModelType{"broken", "scout"}, c.ModelRoles())

	delete(c.Models, SelectedModelTypeOrchestrator)
	_, ok = c.OrchestratorModel()
	require.False(t, ok)

	c.Models[SelectedModelTypeOrchestrator] = SelectedModel{Provider: "dp-claude", Model: "ghost"}
	_, ok = c.OrchestratorModel()
	require.False(t, ok, "unresolvable orchestrator must read as unconfigured")
}

// Config loading validates the orchestrator slot and user roles: a
...
func TestConfigureSelectedModels_RolesValidatedAndBackfilled(t *testing.T) {
	...
	orch, ok := c.Models[SelectedModelTypeOrchestrator]
	require.True(t, ok, "resolvable orchestrator must survive")
	require.Equal(t, "low", orch.ReasoningEffort, "explicit effort wins over the catalog default")
	require.Equal(t, int64(16000), orch.MaxTokens, "max tokens backfilled from the catalog")
