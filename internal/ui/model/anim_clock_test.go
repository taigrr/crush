...
// lowBandwidthTestItem changes its output only every other frame, like a
// per-instance low-bandwidth spinner dividing the fast clock.
type lowBandwidthTestItem struct {
	spinTestItem
	frames int
}

func (l *lowBandwidthTestItem) Advance() bool {
	l.frames++
	if l.frames%2 != 0 {
		return false
	}
...
func TestAnimClock_UnchangedFrameIsScrollOnlyButKeepsClock(t *testing.T) {
	t.Parallel()
	u := newFrameTestUI(t)
	item := &lowBandwidthTestItem{spinTestItem: spinTestItem{testMessageItem: testMessageItem{id: "lb", text: "lb"}, spinning: true}}
