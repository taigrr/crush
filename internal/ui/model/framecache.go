...
func (m *UI) frameCacheable() bool {
	return m.frames != nil && m.state == uiChat && !m.dialog.HasDialogs() && !m.versionMismatch
}
...
func (m *UI) currentFrameKey() (key frameKey, ok bool) {
	...
	offsetIdx, offsetLine := m.chat.ScrollPosition()
	return frameKey{
		width:       m.width,
		height:      m.height,
		offsetIdx:   offsetIdx,
		offsetLine:  offsetLine,
		selectedIdx: m.chat.Selected(),
		focused:     m.chat.Focused(),
	}, true
}
