...
func (a *AssistantMessageItem) Advance() bool {
	...
	// would serve the previously rendered frame indefinitely and the
...
func (a *AssistantMessageItem) SetMessage(msg *message.Message) {
	...
	// need an explicit drop here either. If the message started spinning
	// the UI's animation clock picks it up on the next update.
}
