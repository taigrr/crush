...
import (
	...
	"github.com/taigrr/crush/internal/ui/textarea/internal/memoization"
	"github.com/taigrr/crush/internal/ui/textarea/internal/runeutil"
)
...
type Model struct {
	...
	// highlights are presentational styled ranges; see [Model.SetHighlights].
	highlights []Highlight

	// marks are edit-tracking positions; see marks.go.
	marks   []mark
	markSeq MarkID
}
...
// SetValue sets the value of the text input. Marks are adjusted as for a
// single replacement of the whole buffer: those inside changed text
// collapse onto the edit point.
...
// Reset sets the input to its default state with no input. Marks and
// highlights are dropped along with the text.
func (m *Model) Reset() {
	m.reset()
	m.ClearMarks()
	m.ClearHighlights()
...
// reset clears the buffer, cursor, viewport, and selection but keeps marks
// so a following edit can track them.
func (m *Model) reset() {
...
func (m *Model) view() string {
	...
	for l, line := range m.value {
		...
		for wl, wrappedLine := range wrappedLines {
			...
			// Styled spans (highlights, then the selection on top) are
			// layered over the segment. The inline cursor is drawn inside
			// whichever span it falls in, except during a selection where
			// the highlight is the meaningful affordance.
			spans := m.segmentSpans(l, wrappedBase, len(wrappedLine), styles.computedSelection())
			cursorCol := -1
			if m.row == l && lineInfo.RowOffset == wl {
				if _, _, selected := m.selectionSpanFor(l, wrappedBase, len(wrappedLine)); !selected {
					cursorCol = lineInfo.ColumnOffset
				}
			}
			if cursorCol >= 0 && m.col >= len(line) && lineInfo.CharOffset >= m.width {
				s.WriteString(m.renderSegment(wrappedLine, style, spans, -1))
				m.virtualCursor.SetChar(" ")
				s.WriteString(m.virtualCursor.View())
			} else {
				s.WriteString(m.renderSegment(wrappedLine, style, spans, cursorCol))
			}
